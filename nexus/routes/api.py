...
from middleware.rate_limiter import get_ip_hash, rate_limit
...
def health_check() -> tuple[Response, int]:
    ...
    status_code = HTTP_200_OK if status == "healthy" else HTTP_503_SERVICE_UNAVAILABLE
...
def demo_scenarios() -> tuple[Response, int]:
    """Return pre-built demo scenarios.

    Returns:
        JSON response with demo scenarios.
    """
...
def analyze() -> tuple[Response, int]:
    ...
    safe_text: str | None = sanitize_text(raw_text) if raw_text else None
    safe_context: str | None = sanitize_text(context) if context else None

    # 2. Image processing
    processed_image_bytes: bytes | None = None
    if raw_image:
        image_bytes: bytes = validate_image(raw_image)
    ...
    # 3 & 4. Gemini Analysis
    action_plan = gemini_service.analyze(text=safe_text, image_bytes=processed_image_bytes, context=safe_context)

    # 5 & 6. Logging
    start_time: float = getattr(g, "start_time", time.time())
    duration_ms = int((time.time() - start_time) * 1000)
    client_ip: str | None = request.remote_addr or request.headers.get("X-Forwarded-For", "127.0.0.1")
    ip_hash: str = get_ip_hash(str(client_ip))

    input_preview: str = safe_text if safe_text else "Image upload"
    ...
    response.headers["X-Session-ID"] = getattr(g, "session_id", "unknown")
...
def get_logs() -> tuple[Response, int]:
    ...
    return jsonify({"incidents": incidents, "total": len(incidents)}), HTTP_200_OK


@api_bp.route("/log/<session_id>", methods=["DELETE"])
def delete_log(session_id: str) -> tuple[Response | str, int]:
    """Delete a specific log entry (GDPR).

    Args:
        session_id: The session ID indicating the document to delete.
    ...
    if success:
        ...
        return jsonify({"error": "Log entry not found"}), HTTP_404_NOT_FOUND
